...
let mapToUserDTO = fun (transPayload: TransactionPayload) (b2cUser:B2CUser) (clcsUser: CLCSUser) ->
    task {
        let! apps = getApplicationsByUserIdAsync transPayload clcsUser.IdUser
        ...
        let! users =
            ...
            |> Seq.map (fun user ->
                task {
                    ...
                    return! mapToUserDTO transPayload matchFound user
                }
            )
            |> Task.WhenAll
